...
- (struct rec_str) getOrder //real orders read from a file
{
  ...
  static char  field4[20]="";  // CA/RO/UG
  ...
  static char field8c[20]="";  //minimum planning date
  ...
  char tempMinplan[20];
  char tempMinplanYear[5],tempMinplanMonth[3],tempMinplanDay[3];
  time_t minplan;
...
#define DBG(A) //
    ...
    //field8c (string to to time_t)
    sscanf(field8c,
	   "%[0-9]%*[/]%[0-9]%*[/]%[0-9]",
	   tempMinplanDay,tempMinplanMonth,tempMinplanYear);
    sprintf(tempMinplan,"%s%s%s%s",
	    tempMinplanDay,tempMinplanMonth,tempMinplanYear,timeOfDeliveryDay);

    minplan=[myTimeFilter getAbsoluteTime: tempMinplan];
    ...
    dataToReturn.minplan = minplan; //minimum planning date - S only - time_t
